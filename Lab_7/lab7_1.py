...
def calc_summ_line(T):
    Sum = 0
    for i in range(0, 4):
        Sum += T[i]
    return Sum

def calc_logic_comp(T1, T2):
    Comp = 0
    for i in range(0, 4):
        Comp += int((T1[i] + T2[i])/2)
    return Comp

def cut_double(R, i, Eps, rect) :
    global P1
    global P2
    global Pcp
    global Pm
    ...
    while True:
        ...
        xcp = round((P1[0]+P2[0])/2)
        ycp = round((P1[1]+P2[1])/2)        # вычисление средней точки
        Pcp = [xcp, ycp]                  # вычисление средней точки
        
        Pm = P1                             # запоминание текущей точки P1
        P1 = Pcp

        T1 = get_code(P1, rect)# вычисление нового кода точки P1
        Comp = calc_logic_comp(T1, T2)      # проверка тривиальной невидимости отрезка Pcp P2
...
def middle_point(bar, rect, win):
    ...
    while True:
        ...
        S1 = calc_summ_line(T1)
        S2 = calc_summ_line(T2)

        if(S1 == S2 == 0):
            win.scene.addLine(P1[0], P1[1], P2[0], P2[1], win.pen)
            return

        if(calc_logic_comp(T1, T2) != 0):
            return
        ...
        if(S2 == 0):
            P1 = P2
            P2 = R
            i += 1                          # увеличили счетчик
            continue 


        cut_double(R, i, eps, rect)
